...
const SENT = {
  positive: { dot: "#10b981", border: "rgba(16,185,129,0.28)", label: "BULL", color: "#10b981" },
  negative: { dot: "#f43f5e", border: "rgba(244,63,94,0.28)",  label: "BEAR", color: "#f43f5e" },
  neutral:  { dot: "#64748b", border: "rgba(100,116,139,0.18)", label: "NEU",  color: "#64748b" },
};
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, #0b0b09 0%, #0e0f14 60%, #0b0b09 100%)", boxShadow: "0 0 60px rgba(212,150,58,0.04) inset" }}
      >
        {/* Ambient glow */}
        <div className="absolute top-0 right-0 w-80 h-80 opacity-[0.035] pointer-events-none" style={{ background: "radial-gradient(circle, #d4963a 0%, transparent 65%)", transform: "translate(35%,-35%)" }} />

        <div className="relative flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <span className="text-[9px] font-black uppercase tracking-[0.18em] px-2.5 py-1 rounded-full border" style={{ background: "rgba(212,150,58,0.1)", color: "#d4963a", borderColor: "rgba(212,150,58,0.25)" }}>
                Market Intelligence
              </span>
              {data?.length > 0 && (
                <span className="text-[9px] text-muted font-mono">{data.length} stories collected</span>
              )}
            </div>
            <h1 className="text-2xl font-black text-slate-100 leading-tight mb-1" style={{ fontFamily: "'DM Serif Display', serif" }}>
              Today&apos;s{" "}
              <span style={{ background: "linear-gradient(90deg, #d4963a, #f0c56a)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                Market News
              </span>
            </h1>
            <p className="text-[11px] text-muted">NSE · BSE · Global markets · FII/DII flows · Economy · Sectors</p>
          </div>

          {/* Sentiment meter */}
          {total > 0 && (
            <div className="shrink-0 min-w-[180px]">
              <div className="text-[9px] text-muted uppercase tracking-widest mb-1.5 font-bold">Sentiment Overview</div>
              <div className="flex gap-px h-2 rounded-full overflow-hidden mb-2">
                <div className="bg-emerald-500/60 transition-all duration-700" style={{ width: `${((pos / total) * 100).toFixed(0)}%` }} />
                <div className="bg-slate-600/40 transition-all duration-700" style={{ width: `${((neu / total) * 100).toFixed(0)}%` }} />
                <div className="bg-rose-500/60 transition-all duration-700" style={{ width: `${((neg / total) * 100).toFixed(0)}%` }} />
              </div>
              <div className="flex justify-between text-[9px]">
                <span className="text-emerald-500/70 font-semibold">{pos} bullish</span>
                <span className="text-muted">{neu} neutral</span>
                <span className="text-rose-500/70 font-semibold">{neg} bearish</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none pb-1">
        {NEWS_CATEGORIES.map((c) => (
          ...
          >
            {c.label}
            {counts?.[c.id] != null && (
              <span className="text-[9px] opacity-50 tabular-nums">({counts[c.id]})</span>
            )}
          </button>
        ))}
        <button
      ...
      {isLoading ? (
        ...
        <div className="glass-card flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-xs text-muted">First pull runs at startup — check back in a moment.</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-start gap-2.5 mb-2">
                      <span
                        ...
                        style={{ background: s.dot, boxShadow: `0 0 6px ${s.dot}66` }}
                      />
                      ...
                      <span
                        ...
                        style={{ color: s.color, background: `${s.dot}14`, borderColor: `${s.dot}30` }}
                      >
                        {s.label}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 text-[10px] text-muted flex-wrap ml-4.5">
                      <span
                        ...
                        style={{ background: "rgba(212,150,58,0.1)", color: "#d4963a", border: "1px solid rgba(212,150,58,0.18)" }}
                      >
...
function MobileSearchOverlay({ onClose }) {
  return (
    <div className="mobile-search-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="flex items-center gap-3 pt-4 pb-3">
        <div className="flex-1"><SearchBar autoFocus onResultPicked={onClose} /></div>
        <button onClick={onClose} className="text-sm text-[#7a7060] hover:text-[#ede8df] px-2 py-2 transition-colors">Cancel</button>
      </div>
    </div>
  );
}
...
function NewsRestoreBtn({ onClick }) {
  return (
    <button
      ...
      style={{ background: "rgba(17,17,16,0.96)", backdropFilter: "blur(12px)", border: "1px solid rgba(212,150,58,0.28)", color: "#7a7060", boxShadow: "0 8px 32px rgba(0,0,0,0.4)", fontFamily: "'DM Mono', monospace" }}
    >
      <span className="text-sm">📰</span>
      <span className="uppercase tracking-wider text-[10px] group-hover:text-[#d4963a] transition-colors">News Feed</span>
...
export default function App() {
  ...
  const gainers50   = livePrices?.gainers?.length > 0 ? livePrices.gainers : gainers50Data;
  ...
  const isAnalysisPage = analysisFlag === "1" || analysisFlag === "true" || window.location.pathname === "/analysis";
    ...
    return (
      <div className="min-h-screen max-h-screen flex flex-col bg-ink overflow-hidden">
        <div className="px-4 py-2 text-[11px] text-[#7a7060] border-b border-[rgba(212,150,58,0.08)] bg-[rgba(212,150,58,0.03)] flex items-center gap-2 flex-wrap">
          ...
          <span>·</span>
          <span className="text-[#b8af9e] font-mono">{analysisSymbol || <span className="text-red-400">no symbol</span>}</span>
        </div>
  ...
  const gridCols =
    activeTab === "news" ? "md:grid-cols-1"
    : newsCollapsed || activeTab === "sectors" || activeTab === "guide" ? "md:grid-cols-1"
    : "md:grid-cols-3";
  ...
  return (
    <div className="min-h-full flex flex-col bg-ink">
      ...
      <header className="sticky top-0 z-20 glass-card rounded-none border-t-0 border-l-0 border-r-0">
        <div className="max-w-[1600px] mx-auto px-3 sm:px-4 py-2.5 flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <img src="/logo.png" alt="MarketPulse Logo" className="w-8 h-8 rounded-lg shadow-amber-md object-cover shrink-0" />
            <div className="hidden xs:block">
              <h1 className="text-sm font-bold gradient-text leading-none" style={{ fontFamily: "'DM Serif Display', serif", letterSpacing: "-0.01em" }}>
                MarketPulse
              </h1>
              <div className="text-[8px] text-[#4a4540] leading-tight tracking-widest uppercase" style={{ fontFamily: "'DM Mono', monospace" }}>
                NSE · BSE · AI
              </div>
            </div>
          </div>
          ...
          {/* Market status */}
          <div className={`hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border transition-all duration-300 shrink-0 ${
            isOpen ? "bg-bull/10 text-bull border-bull/20" : "bg-[#111110]/80 text-[#7a7060] border-[rgba(212,150,58,0.12)]"
          }`}>
            {isOpen ? <span className="live-dot" /> : <span className="inline-block w-2 h-2 rounded-full bg-[#333]" />}
          ...
          {activeTab === "markets" && (
            <button onClick={() => setNewsCollapsed((v) => !v)}
          ...
          <button onClick={() => setPortfolioOpen(true)} className="hidden sm:flex btn-ghost text-xs items-center gap-1.5 !min-h-[36px] !py-1.5">
            <span>💼</span><span className="hidden lg:inline">Portfolio</span>
          </button>
          <button onClick={() => setImpactOpen(true)} className="btn-primary text-xs !min-h-[36px] !py-1.5 !px-3">
            <span>⚡</span><span className="hidden sm:inline ml-1">Impact</span>
          </button>
        </div>
      </header>
      ...
      {/* ── Main ─────────────────────────────────────────────────────── */}
      <main className={`max-w-[1600px] mx-auto w-full px-3 sm:px-4 py-3 sm:py-4 flex-1 pb-[calc(var(--bottom-nav-h)+1rem)] md:pb-4 md:grid md:gap-4 ${gridCols}`}>

        {/* NEWS TAB — full width, own layout */}
        {activeTab === "news" && (
          <div className="md:col-span-1">
            {/* Mobile: compact feed */}
            <div className="block md:hidden"><NewsFeed /></div>
            {/* Desktop: rich 2-col grid */}
            <div className="hidden md:block"><FullWidthNewsFeed /></div>
          </div>
        )}
      ...
      {newsCollapsed && activeTab === "markets" && <NewsRestoreBtn onClick={() => setNewsCollapsed(false)} />}
      ...
      <nav className="mobile-bottom-nav md:hidden" role="navigation" aria-label="Main navigation">
        ...
        <button onClick={() => setPortfolioOpen(true)} className="mobile-nav-btn" aria-label="Portfolio">
          <span className="mobile-nav-btn-icon">💼</span>
          <span className="mobile-nav-btn-label">Portfolio</span>
        </button>
      </nav>

      {mobileSearchOpen && <MobileSearchOverlay onClose={() => setMobileSearch(false)} />}
      ...
      <footer className="hidden md:block border-t border-[rgba(212,150,58,0.08)] py-2">
        <div className="max-w-[1600px] mx-auto px-4 text-[10px] text-[#7a7060] flex items-center justify-between" style={{ fontFamily: "'DM Mono', monospace" }}>
          <span>MarketPulse · NSE/BSE · AI via Ollama</span>
          <span className="text-rose-600/50 font-medium">Not financial advice</span>
        </div>
      </footer>
    </div>
  );
